...
with open('city_area_road.pkl', 'rb') as file:
    ...
    # 製作最後存資料的datafram，分3個欄位，分別為：縣市名、地區名、公車站牌數
    # 先製作多列清單
    row_name = []
    for key in road:
...
for city in range(4,city_num):
        ...
        for j in range(head, tail):
            ...
            for k in range(len(road_list)):
                ...
                for sec in range(len(section_num_1)):
                    ...
                    elif section_num_2[sec] in road_list[k]:
    ...
    if city < 21:
        if city == 3:
            ...
            head += len(road[city_list[city]])
            tail += len(road[city_list[city + 1]])
# df.to_csv(path_or_buf = '/Users/wangyuda/Desktop/project/Bus/公車站牌.csv')
